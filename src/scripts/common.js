...
/* infoBar */
const closeInfoBar = document.querySelectorAll(".js-close-infobar");
...
function checkCookie(name) {
  let param = getCookie(name);

  if (param == 'true') {
    infoBar.classList.add("hide-infobar");
  } else {
    infoBar.classList.remove("hide-infobar");
  }
}

closeInfoBar.forEach((item) => {
  item.addEventListener("click", function () {
    infoBar.classList.add("hide-infobar");
    setCookie('hideInfoBar', true)
  });
});

document.addEventListener("DOMContentLoaded", () => {
  checkCookie('hideInfoBar');
});
